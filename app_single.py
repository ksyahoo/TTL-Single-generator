...
# 設定頁面資訊
st.set_page_config(page_title="AI 電商圖卡生成器 (單商品版)", layout="centered")
...
def create_composite_image(bg_img, prod1_img, text1, text2, btn_text, font_source):
    # 1. 準備背景
    original_bg = bg_img
    ...
    if len(images) >= 1:
        ...
        # 單商品邏輯：水平置中
        p1_x = int((CANVAS_WIDTH - p1.width) / 2)
        
        bg.paste(p1, (p1_x, start_y), p1)
...
with col1:
    ...
    # 移除商品圖 B，只保留 A
    p1_img = image_input_area("商品圖", "p1")
...
if st.button("生成圖片"):
    ...
    elif bg_img and p1_img:
        try:
            # 呼叫函式時只傳入一張商品圖
            final, detected_color = create_composite_image(bg_img, p1_img, text_line1, text_line2, btn_text, font_upload)
